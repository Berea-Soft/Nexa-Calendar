...
import type { CalendarState, ViewType, DayCell, DayColumn, ListEvent, TimelineGrid, ResourceTimelineGrid, YearGrid, EventInput, ICalendarEvent, IEventSource, EventSourceRawInput, ICalendarPlugin, Locale, BusinessHours, DropPayload, IResource, ResourceInput } from '@nexa-calendar/core'
...
export class NxCalendar extends LitElement {
  ...
  @state() private _grid: DayCell[] | DayColumn[] | ListEvent[] | TimelineGrid | ResourceTimelineGrid | YearGrid = []
  ...
  @property({ type: Array }) set resources(value: ResourceInput[]) {
    if (value) this._resources = value.map(r => ({ ...r, children: r.children ?? [] } as IResource))
  }
  ...
  @property({ type: Array }) views: ViewType[] = ['month', 'workWeek', 'week', 'day', 'list', 'timeline', 'timelineDay', 'timelineWeek', 'timelineMonth', 'year']
  ...
  private _getRange(date: import('@bereasoftware/time-guard').TimeGuard, v: ViewType): { start: import('@bereasoftware/time-guard').TimeGuard; end: import('@bereasoftware/time-guard').TimeGuard } {
    switch (v) {
      case 'month': return DateUtils.getMonthRange(date)
      case 'workWeek': return DateUtils.getWeekRange(date)
      case 'week': return DateUtils.getWeekRange(date)
      case 'day': return DateUtils.getDayRange(date)
      case 'list': return DateUtils.getMonthRange(date)
      case 'timeline': return DateUtils.getTimelineRange(date)
      case 'timelineDay': return DateUtils.getDayRange(date)
      case 'timelineWeek': return DateUtils.getWeekRange(date)
      case 'timelineMonth': return DateUtils.getTimelineRange(date)
      case 'year': return { start: date.startOf('year'), end: date.endOf('year') }
    }
  }
  ...
  private _handleDateClick(e: CustomEvent): void {
    const detail = e.detail as { date: import('@bereasoftware/time-guard').TimeGuard; allDay: boolean }
  ...
  private _handleEventDrop(e: CustomEvent): void {
    const { eventId, newStart, newEnd } = e.detail as { eventId: string | number; newStart: string; newEnd?: string }
  ...
  private _getLocale(): Locale {
    try { return getLocale(this.locale) }
    catch { return getLocale('en') }
  }
  ...
  private get _bizHoursKey(): string {
    ...
    if (Array.isArray(this.businessHours)) {
      return this.businessHours.map(h => `${h.daysOfWeek?.join('')}-${h.startTime}-${h.endTime}`).join('|')
    }
  ...
  render() {
    ...
    return html`
      <div class="nx-calendar-root rounded-xl overflow-hidden relative"
        ...
        ${isFetching ? html`
          <div class="absolute inset-0 flex items-center justify-center z-50" style="background: color-mix(in srgb, var(--nx-bg) 80%, transparent)">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2" style="border-color: var(--nx-accent)"></div>
          </div>
        ` : ''}

        ${error ? html`
          <div class="px-4 py-2 text-sm font-medium" style="background: #fef2f2; border-bottom: 1px solid #fecaca; color: #991b1b">${error}</div>
        ` : ''}
        ...
        <div class="p-0">
          ...
          ` : view === 'timeline' ? html`
            <nx-timeline-view
              ...
              @eventDrop=${this._handleEventDrop}
            ></nx-timeline-view>
          ` : view === 'timelineDay' || view === 'timelineWeek' || view === 'timelineMonth' ? html`
      ...
      <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
        ...
        <div class="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl"
          ...
          <div class="px-5 py-4 flex items-start justify-between gap-3"
            ...
            <div>
              <div class="text-base font-bold leading-snug" style="color: ${event.textColor ?? '#fff'}">${event.title}</div>
              ${event.allDay ? html`<span class="mt-1 inline-block text-xs font-medium px-2 py-0.5 rounded-full bg-white/20" style="color: ${event.textColor ?? '#fff'}">All day</span>` : ''}
            </div>
            <button class="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/30 transition-colors text-sm font-bold"
              style="color: ${event.textColor ?? '#fff'}"
              @click=${() => { this._modalEvent = null }}>✕</button>
          </div>
          <!-- Detail rows -->
          <div class="px-5 py-4 space-y-3" style="color: var(--nx-text)">
            ...
            ${event.extendedProps && Object.keys(event.extendedProps).length ? html`
              <div class="border-t pt-3" style="border-color: var(--nx-border-light)">
                ${Object.entries(event.extendedProps).map(([k, v]) => html`
                  <div class="flex gap-2 text-xs">
                    <span class="font-medium capitalize" style="color: var(--nx-text-muted)">${k}:</span>
                    <span>${String(v)}</span>
                  </div>
                `)}
              </div>
          ...
          <div class="px-5 pb-4 flex gap-2 justify-end">
            <button class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              ...
              @click=${() => { this._modalEvent = null }}>Close</button>
            ${this.editable ? html`
              <button class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                ...
                @click=${() => {
                  ...
                  this.dispatchEvent(new CustomEvent('editEvent', { detail: { id, event }, bubbles: true, composed: true }))
                }}>Edit</button>
              <button class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
