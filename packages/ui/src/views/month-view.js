      ...
      <div class="grid grid-cols-7" style="gap: 1px; background: var(--nx-border);">
        ...
        ${weeks.flatMap((_week, _wi) => _week.map((cell, ci) => {
            ...
            <div style="background: ${cellBg}; ${todayRing} min-height: 100px; opacity: ${isBiz ? 1 : 0.6}; position: relative; padding: 6px; cursor: pointer; transition: background 0.15s;"
              ...
              @click=${() => this._selectDate(cell)} @dragover=${(e) => e.preventDefault()} @drop=${(e) => this._onDrop(e, cell)}>
              ...
              <div style="margin-top: 4px; display: flex; flex-direction: column; gap: 2px;">
                ...
                : `background: ${e.backgroundColor ?? 'var(--nx-accent)'}; color: ${e.textColor ?? '#fff'}; border-radius: 4px; padding: 2px 6px;`}
                    ...
                    ${e.display === 'background' ? e.title : html `${e.allDay ? '' : html `<span style="opacity:0.8">${this._formatTime(e.start)}</span>`}${e.title}`}
                  </div>
                `)}
                ${hasMore ? html `
                  <div style="font-size: 0.72rem; color: var(--nx-accent); font-weight: 600; padding: 0 4px; cursor: pointer;"
                    ...
                    @click=${(ev) => { ev.stopPropagation(); this._togglePopover(cell); }}>
        ...
        <div style="max-height: 180px; overflow-y: auto; padding: 6px; display: flex; flex-direction: column; gap: 3px;">
          ${cell.events.map(e => html `
            ...
            : `background: ${e.backgroundColor ?? 'var(--nx-accent)'}; color: ${e.textColor ?? '#fff'}; border-radius: 4px;`}
              ...
              @mouseenter=${(ev) => ev.currentTarget.style.opacity = '0.8'}
              @mouseleave=${(ev) => ev.currentTarget.style.opacity = '1'}
              @click=${(ev) => { ev.stopPropagation(); this._eventClick(ev, e); this._popoverCell = null; }}>
