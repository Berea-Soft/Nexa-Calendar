...
export class NxMonthView extends LitElement {
  ...
  render() {
    ...
    const maxEvents = this.dayMaxEvents === true ? Infinity : (this.dayMaxEvents as number) || Infinity

    return html`
      <div class="grid grid-cols-7" style="gap: 1px; background: var(--nx-border);">
        ${weekdays.map(d => html`
          <div style="background: var(--nx-header-bg); color: var(--nx-text-muted);" class="px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider">${d}</div>
        `)}
        ${weeks.flatMap((_week, _wi) => _week.map((cell, ci) => {
          ...
          return html`
            <div style="background: ${cellBg}; ${todayRing} min-height: 100px; opacity: ${isBiz ? 1 : 0.6}; position: relative; padding: 6px; cursor: pointer; transition: background 0.15s;"
              @mouseenter=${(e: MouseEvent) => (e.currentTarget as HTMLElement).style.background = `var(--nx-hover)`}
              @mouseleave=${(e: MouseEvent) => (e.currentTarget as HTMLElement).style.background = cellBg}
              @click=${() => this._selectDate(cell)} @dragover=${(e: DragEvent) => e.preventDefault()} @drop=${(e: DragEvent) => this._onDrop(e, cell)}>
              <span style="font-size: 0.8rem; font-weight: 600; color: ${cell.isToday ? 'var(--nx-accent)' : cell.isCurrentMonth ? 'var(--nx-text)' : 'var(--nx-text-faint)'};">
                ${cell.isToday
                  ? html`<span style="background: var(--nx-accent); color: var(--nx-accent-text); border-radius: 999px; width: 1.5rem; height: 1.5rem; display: inline-flex; align-items: center; justify-content: center; font-size: 0.75rem;">${cell.date.day()}</span>`
                  : cell.date.day()}
              </span>
              <div style="margin-top: 4px; display: flex; flex-direction: column; gap: 2px;">
                ${visibleEvents.map(e => html`
                  <div style="${e.display === 'background'
                    ...
                    @mouseenter=${(ev: MouseEvent) => (ev.currentTarget as HTMLElement).style.opacity = '0.8'}
                    @mouseleave=${(ev: MouseEvent) => (ev.currentTarget as HTMLElement).style.opacity = '1'}
                    @click=${(ev: Event) => { ev.stopPropagation(); this._eventClick(ev, e) }}
                    @dragstart=${(ev: DragEvent) => this._onEventDragStart(ev, e)}>
                    ${e.display === 'background' ? e.title : (() => { const content = this._renderEventContent(e); return html`<span class=${content.className}>${content.html}</span>` })()}
                  </div>
                `)}
                ${hasMore ? html`
                  <div style="font-size: 0.72rem; color: var(--nx-accent); font-weight: 600; padding: 0 4px; cursor: pointer;"
                    ...
                    @click=${(ev: Event) => { ev.stopPropagation(); this._togglePopover(cell) }}>
  ...
  private _renderPopover(cell: DayCell) {
    return html`
      <div style="position: absolute; z-index: 50; top: calc(100% + 4px); left: 0; width: 240px; background: var(--nx-surface); border: 1px solid var(--nx-border); border-radius: var(--nx-radius); box-shadow: var(--nx-shadow);"
        @click=${(ev: Event) => ev.stopPropagation()}>
        <div style="padding: 8px 12px; border-bottom: 1px solid var(--nx-border-light); font-size: 0.8rem; font-weight: 600; color: var(--nx-text);">${cell.date.format('dddd D')}</div>
        <div style="max-height: 180px; overflow-y: auto; padding: 6px; display: flex; flex-direction: column; gap: 3px;">
          ${cell.events.map(e => html`
            <div style="${e.display === 'background'
              ...
              @mouseenter=${(ev: MouseEvent) => (ev.currentTarget as HTMLElement).style.opacity = '0.8'}
              @mouseleave=${(ev: MouseEvent) => (ev.currentTarget as HTMLElement).style.opacity = '1'}
              @click=${(ev: Event) => { ev.stopPropagation(); this._eventClick(ev, e); this._popoverCell = null }}>
              ${e.display === 'background' ? e.title : (() => { const content = this._renderEventContent(e); return html`<span class=${content.className}>${content.html}</span>` })()}
            </div>
          `)}
        </div>
  ...
  private _selectDate(cell: DayCell) {
    this.dispatchEvent(new CustomEvent('dateClick', { detail: { date: cell.date, allDay: true }, bubbles: true, composed: true }))
  }
  ...
  private _onDrop(e: DragEvent, cell: DayCell) {
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('eventDrop', { detail: { eventId, newStart: cell.date.toISOString() }, bubbles: true, composed: true }))
    } catch { /* ignore */ }
  }
