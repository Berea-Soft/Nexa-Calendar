      ...
      <div style="overflow: auto; max-height: 500px;">
        <div style="min-width: ${columns.length * colWidth}px">
          ...
          <div style="min-height: 100px; background: var(--nx-bg);">
            <div
              style="position: relative; height: ${Math.max(
                100,
                eventRows.length * rowHeight
              )}px; min-width: ${columns.length * colWidth}px;"
            >
              <!-- Grid lines -->
              ${columns.map(
                (col, i) =>
              ...
              <!-- Time slots -->
              ${Array.from({ length: Math.ceil((eventRows.length * rowHeight) / 60) }).map(
                (_, slotIdx) => html`
                  <div
...
NxTimelineView = __decorate([customElement('nx-timeline-view')], NxTimelineView);
