              ...
              <div style="display: flex; align-items: flex-start; gap: 16px; padding: 10px 16px; cursor: pointer; border-left: 3px solid ${item.event.backgroundColor ?? 'var(--nx-accent)'}; transition: background 0.1s;"
                ...
                <div style="flex: 1; min-width: 0;">
                  <div style="display: flex; align-items: center; gap: 8px;">
                    ${(() => { const content = this._renderEventContent(item.event); return html`<span class=${content.className} style="font-size: 0.875rem; font-weight: 600; color: var(--nx-text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${content.html}</span>` })()}
                  ...
                  ${item.event.extendedProps?.description ? html`<p style="font-size: 0.75rem; color: var(--nx-text-muted); margin: 2px 0 0; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${item.event.extendedProps.description as string}</p>` : ''}
                </div>
              </div>`)}
          </div>`)}
      </div>`
