              ...
              ${timeSlots.map(slot => {
                ...
                return html` <div
                  style="height: ${slotHeight}px; border-bottom: 1px solid var(--nx-border-light); position: relative; background: ${isBiz
                    ? 'transparent'
                    : 'var(--fc-non-business-color)'}; transition: background 0.1s; cursor: ${isBiz
                    ? 'pointer'
                    : 'not-allowed'}; opacity: ${isBiz ? '1' : '0.72'};"
                  @mouseenter=${e => {
                    if (isBiz) e.currentTarget.style.background = 'var(--nx-hover)';
                  }}
                  @mouseleave=${e => {
...
NxWeekView = __decorate([customElement('nx-week-view')], NxWeekView);
export { NxWeekView };
