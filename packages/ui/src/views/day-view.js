          ...
          ${timeSlots.map(slot => {
            ...
            return html` <div
              ...
              @mouseenter=${e => {
                ...
                if (isBiz) el.style.background = 'var(--nx-hover)';
              }}
              @mouseleave=${e => {
                const el = e.currentTarget;
            ...
            >
              ${events.map(
                ev =>
                  ...
                  >
                    <strong>${ev.title}</strong>${ev.end
                      ? html` <span style="opacity:0.8;margin-left:4px"
                          >${this._formatTime(ev.start)} – ${this._formatTime(ev.end)}</span
                        >`
                      : ''}
                  </div>`
              )}
...
NxDayView = __decorate([customElement('nx-day-view')], NxDayView);
export { NxDayView };
