...
export function buildVisualTimeSlots(
  minTime: string,
  maxTime: string,
  slotDuration: number,
...
export function getTimedEventsForSlot(
...
export function getBackgroundEventsForSlot(
  events: ICalendarEvent[],
  slotStartMinutes: number,
  slotDuration: number
): ICalendarEvent[] {
  ...
  return events.filter(event => {
    ...
    const eventStartMinutes = event.start.hour() * 60 + event.start.minute();
    const eventEndMinutes = event.end
      ? event.end.hour() * 60 + event.end.minute()
      : eventStartMinutes + slotDuration;
