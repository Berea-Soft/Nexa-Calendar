...
export class EventSorter {
  ...
  private _defaultSort(events: ICalendarEvent[]): ICalendarEvent[] {
    return [...events].sort((a, b) => {
  ...
  private _sortBy(events: ICalendarEvent[], configs: SortConfig[]): ICalendarEvent[] {
    return [...events].sort((a, b) => {
      for (const { key, direction } of configs) {
        ...
        } else if (aVal instanceof Object && 'valueOf' in (aVal as object)) {
          cmp = (aVal as unknown as { valueOf(): number }).valueOf() - (bVal as unknown as { valueOf(): number }).valueOf()
        } else {
