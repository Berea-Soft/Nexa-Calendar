...
import type { ViewType, DayCell, DayColumn, ListEvent, IView, TimelineGrid, TimelineColumn, TimelinePositionedEvent, TimelineEventRow, ResourceTimelineGrid, ResourceTimelineRow, YearGrid, YearMonth, YearMonthCell } from '../types/view'
...
class TimelineView implements IView {
  ...
  getTitle(range: { start: TimeGuard; end: TimeGuard }, locale?: string): string {
    ...
    if (locale) {
      ...
      if (days <= 7) return `${s.locale(locale).format('MMM D')} – ${e.locale(locale).format('MMM D, YYYY')}`
      return `${s.locale(locale).format('MMMM YYYY')}`
    }
    return `Timeline: ${s.format('date')} – ${e.format('date')}`
  }

  generateGrid(range: { start: TimeGuard; end: TimeGuard }, events: ICalendarEvent[], _resources?: IResource[]): TimelineGrid | ResourceTimelineGrid {
  ...
  protected _generateColumns(range: { start: TimeGuard; end: TimeGuard }, durationDays: number): TimelineColumn[] {
    ...
    if (durationDays <= 1) {
      while (cursor.isBefore(range.end)) {
        cols.push({ date: cursor, label: cursor.format('h:mm a'), isToday: cursor.isSame(today, 'day'), isWeekend: false })
        cursor = cursor.add({ hour: 1 })
      }
    } else {
      while (cursor.isBefore(range.end) || cursor.isSame(range.end, 'day')) {
        const dow = cursor.dayOfWeek()
        cols.push({ date: cursor, label: cursor.format('ddd D'), isToday: cursor.isSame(today, 'day'), isWeekend: dow === 0 || dow === 6 })
  ...
  protected _positionEvents(events: ICalendarEvent[], range: { start: TimeGuard; end: TimeGuard }): TimelinePositionedEvent[] {
  ...
  protected _layoutEvents(events: ICalendarEvent[], range: { start: TimeGuard; end: TimeGuard }, _cols: TimelineColumn[]): TimelineEventRow[] {
    const positioned = this._positionEvents(events, range)
    const rows: TimelineEventRow[] = []
    for (const p of positioned) {
      let placed = false
      for (const row of rows) {
        const overlaps = row.events.some(r => p.leftPct < r.leftPct + r.widthPct && p.leftPct + p.widthPct > r.leftPct)
        if (!overlaps) { row.events.push(p); placed = true; break }
      }
...
class ResourceTimelineDayView extends TimelineView {
  ...
  override generateGrid(range: { start: TimeGuard; end: TimeGuard }, events: ICalendarEvent[], resources: IResource[] = []): TimelineGrid | ResourceTimelineGrid {
    return this._buildResourceGrid(range, events, resources, 1)
  }

  protected _buildResourceGrid(range: { start: TimeGuard; end: TimeGuard }, events: ICalendarEvent[], resources: IResource[], durationDays: number): ResourceTimelineGrid {
    const columns = this._generateColumns(range, durationDays)
    const rows = this._buildRows(resources, events, range, 0)
    return { columns, rows }
  }

  protected _buildRows(resources: IResource[], events: ICalendarEvent[], range: { start: TimeGuard; end: TimeGuard }, depth: number): ResourceTimelineRow[] {
...
class ResourceTimelineWeekView extends ResourceTimelineDayView {
  ...
  override getTitle(range: { start: TimeGuard; end: TimeGuard }, locale?: string): string {
    const s = range.start; const e = range.end
    return locale ? `${s.locale(locale).format('MMM D')} – ${e.locale(locale).format('MMM D, YYYY')}` : `${s.format('date')} – ${e.format('date')}`
  }
...
class TimelineWorkWeekView extends TimelineView {
  ...
  override generateGrid(range: { start: TimeGuard; end: TimeGuard }, events: ICalendarEvent[]): TimelineGrid {
...
export class ViewManager {
  ...
  get currentType(): ViewType { return this._currentType }
