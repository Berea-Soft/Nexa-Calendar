...
class MonthView {
    ...
    getTitle(range, locale) {
        const d = range.start;
        if (locale)
            return `${d.locale(locale).format('MMMM')} ${d.year()}`;
...
class DayView {
    ...
    getTitle(range, locale) {
        const d = range.start;
        if (locale)
            return d.locale(locale).format('dddd, MMMM D, YYYY');
...
class TimelineView {
    ...
    getTitle(range, locale) {
        ...
        if (locale) {
            if (days <= 1)
                return s.locale(locale).format('dddd, MMMM D, YYYY');
            if (days <= 7)
                return `${s.locale(locale).format('MMM D')} – ${e.locale(locale).format('MMM D, YYYY')}`;
...
export class ViewManager {
    ...
    getView(type) {
        ...
        if (!this._instances.has(t)) {
            ...
            if (!factory)
                throw new Error(`View "${t}" not registered`);
