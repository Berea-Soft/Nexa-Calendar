...
export type ViewType = 'month' | 'workWeek' | 'week' | 'day' | 'list' | 'timeline' | 'timelineDay' | 'timelineWeek' | 'timelineWorkWeek' | 'timelineMonth' | 'year'
...
export interface IView {
  ...
  generateGrid(range: DateRange, events: ICalendarEvent[], resources?: IResource[]): DayCell[] | DayColumn[] | ListEvent[] | TimelineGrid | ResourceTimelineGrid | YearGrid
  getViewOptions?(): Partial<ViewOptions>
}

export type ViewTitleMap = Record<ViewType, (date: TimeGuard) => string>

export type TimelineDuration = 'month' | 'week' | 'workWeek' | 'day'
